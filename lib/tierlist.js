...
export async function loadDefaultTierList() {
  try {
    ...
    if (response.ok) {
      ...
      console.log('Loaded default tier list from GitHub:', data) // Debug log
      const converted = convertGitHubFormatToInternal(data)
      console.log('Converted default tier list:', converted) // Debug log
      return converted
    }
  } catch (err) {
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json()
      console.log('Loaded default tier list from fallback:', data) // Debug log
      const converted = convertGitHubFormatToInternal(data)
      console.log('Converted default tier list from fallback:', converted) // Debug log
      return converted
    }
  } catch (err) {
...
export async function loadMercTierList() {
  try {
    ...
    if (response.ok) {
      const data = await response.json()
      console.log('Loaded merc tier list from GitHub:', data) // Debug log
      const converted = convertGitHubFormatToInternal(data)
      console.log('Converted merc tier list:', converted) // Debug log
      return converted
    }
  } catch (err) {
...
export function saveTierList(tierListId, tierListData) {
  ...
  const existingTierList = tierLists[tierListId]
  
  // Don't allow saving changes to read-only tier lists, except for title/name updates if they're "Untitled"
  if (existingTierList?.isReadOnly) {
    // Allow updating title/name if current title is "Untitled" or missing
    const isTitleUpdate = (existingTierList.title === 'Untitled' || !existingTierList.title) && 
                          tierListData.title && 
                          tierListData.title !== 'Untitled'
    if (!isTitleUpdate) {
      return false
    }
  }
...
export function createTierList(name, baseTierList = null, useDefaultId = false, isReadOnly = false, fixedId = null) {
  ...
  // If using a fixed ID and it already exists, update it if it has "Untitled" title
  if ((useDefaultId || fixedId) && tierLists[newId]) {
    const existing = tierLists[newId]
    // If existing tier list has "Untitled" or no title, and we have a title in baseTierList, update it
    if (baseTierList && (existing.title === 'Untitled' || !existing.title || existing.name === 'Untitled' || !existing.name)) {
      const newTitle = baseTierList.title || baseTierList.name || name
      if (newTitle && newTitle !== 'Untitled') {
        tierLists[newId] = {
          ...
          name: newTitle,
          title: newTitle,
          isReadOnly: isReadOnly || existing.isReadOnly || false
        }
  ...
  const newTierList = baseTierList ? {
    ...baseTierList,
    name: baseTierList.title || baseTierList.name || name,
    title: baseTierList.title || baseTierList.name || name
  } : {
    name,
    title: name,
    version: '1.0.0',
  ...
  tierLists[newId] = {
    ...newTierList,
    name: newTierList.name || newTierList.title || name,
