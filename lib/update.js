...
async function fetchKillteamFilesList(locale = DEFAULT_LOCALE) {
  // Use GitHub API to list files in the teams directory
  try {
    const localePath = locale === 'en' ? 'en' : locale
    const apiUrl = `https://api.github.com/repos/xDavidLeon/killteamjson/contents/${localePath}/teams`
    const res = await fetch(apiUrl, { cache: 'no-store' })
    if (!res.ok) {
      console.warn(`Failed to list team files (${res.status}) for locale ${locale}`)
    ...
    console.warn(`Error listing team files for locale ${locale}:`, err)
    return [] // Return empty array instead of throwing
  }
}

async function fetchKillteamDatasetText(locale = DEFAULT_LOCALE) {
  // Fetch all individual team files and combine them
  try {
    const teamFiles = await fetchKillteamFilesList(locale)
    const localePath = locale === 'en' ? 'en' : locale
    const teams = []
...
async function ensureKillteamData({ force = false, locale = DEFAULT_LOCALE } = {}) {
  try {
    ...
    const text = await fetchKillteamDatasetText(locale)
    const hash = await hashText(text)
