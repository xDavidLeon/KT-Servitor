...
import { db } from './db'
import { getLocalePath } from './update'
...
async function buildSearchDocs(locale = 'en') {
  ...
  if (!cachedUniversalActions || !cachedMissionActions) {
    try {
      // Fetch merged actions.json instead of separate files
      const json = await fetchJson(getLocalePath(locale, 'actions.json'))
      let allActions = []
      ...
      // Split into universal and mission based on type field
      if (!cachedUniversalActions) {
        cachedUniversalActions = allActions.filter(action => {
          const type = (action.type || '').toLowerCase()
          return !type || type === 'universal' || type === ''
        })
      }
      if (!cachedMissionActions) {
        cachedMissionActions = allActions.filter(action => {
          const type = (action.type || '').toLowerCase()
          return type === 'mission'
        })
      }
  ...
  if (!cachedWeaponRules) {
    try {
      const json = await fetchJson(getLocalePath(locale, 'weapon_rules.json'))
  ...
  if (!cachedOperations) {
    try {
      const json = await fetchJson(getLocalePath(locale, 'ops_2025.json'))
  ...
  if (!cachedEquipmentActions) {
    try {
      const json = await fetchJson(getLocalePath(locale, 'universal_equipment.json'))
